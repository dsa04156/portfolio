...
// Pointer-only depth: no perpetual animation, timers, or rendering loop.
const hero = document.querySelector('.precision-hero');
const heroArt = document.querySelector('.precision-art');
const finePointer = window.matchMedia('(hover: hover) and (pointer: fine)');
function resetHeroDepth() {
  heroArt.style.removeProperty('--art-x');
  heroArt.style.removeProperty('--art-y');
}
hero.addEventListener('pointermove', event => {
  if (reducedMotion.matches || !finePointer.matches || event.pointerType === 'touch') return;
  const bounds = hero.getBoundingClientRect();
  heroArt.style.setProperty('--art-x', `${((event.clientX - bounds.left) / bounds.width - .5) * 18}px`);
  heroArt.style.setProperty('--art-y', `${((event.clientY - bounds.top) / bounds.height - .5) * 12}px`);
});
